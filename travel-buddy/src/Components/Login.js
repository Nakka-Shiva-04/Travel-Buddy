...
function Login() {
  ...
  const handleRequest = async (e:any) => {
    ...
    setShowNoT(false);
    try {
      axios.get("http://localhost:1234/requestCheck?username=" + name )
      .then((res) => {
        if (res.data == "success") {
  ...
  const handleYes = async (e) => {
    e.preventDefault();
    try {
      await axios.delete("http://localhost:1234/deleteapplication?username=" + oTraveler );
      await axios.delete("http://localhost:1234/deleteapplication?username=" + tTraveler );
      await axios.delete("http://localhost:1234/requestResponse?username=" + name );
  ...
  const handleSubmit = (e) => {
    ...
    try {
      axios.get("http://localhost:1234/login?username=" + name + "&password=" + password )
      .then((res) => {
        if (res.data == "success") {
          setShowLogin(false);
          axios.get("http://localhost:1234/tripCheck?username=" +name)
          .then((res) => {
            if (res.data == "exists") {
              setShowUser(true);
              setTripJoined(true);
  ...
  const handleExit = () => {
    try {
      axios.delete("http://localhost:1234/deletetrip?username=" + name );
      setShowUser(false);
      setShowNoT(true);
    } catch (error) {
      console.log(error);
              ...
              if (res.data == "success") {
                ...
                .then((res) => {
                  if (res.data == "success") {
                    ...
                    axios.get("http://localhost:1234/requestOne?username=" + requestFrom)
                    .then((res) => {
                      if (res.data == "success") {
                        setShowInput(false);
                        ...
                        if (requestTo == requestFrom) {
                          ...
                          const response = axios.post("http://localhost:1234/requests",request )
                          .then((res) => {
                            setShowInput(false);
          ...
          if (res.data == "exists") {
            ...
            .then((res) => {
              if (res.data == "success") {
                ...
                axios.get("http://localhost:1234/tripCheck?username=" + userName)
                .then((res) => {
                  if (res.data == "exists") {
                    setShowJoined(true);
                  } else {
